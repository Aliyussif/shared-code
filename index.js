...
let urlETHEREUM = "https://api.etherscan.io/api?module=gastracker&action=gasoracle&apikey=AH5X2XBJAIHTANBQCJ2IYIBFPQ8XU7TD2Z";
let urlBINANCE = "https://api.bscscan.com/api?module=gastracker&action=gasoracle&apikey=8CDH18MTMFC22N5AUAIDPXZMGES4HBMQAG";
...
const web3ETH = new Web3('https://eth-goerli.g.alchemy.com/v2/yIqkLYrjPSKrTrGULHM-MSbGdzPb94xo');
const web3BSC = new Web3('https://data-seed-prebsc-1-s1.binance.org:8545/');

const contractUSDT = "TUooCBiFRvRjVJiom4j9ywoi1sX9NTAUwq";
const contractBscUSDT = "0xff981a6afad78a38bccdb73e68a3d4c4de72606a";
const contractEthUSDT = "0x01245DD1A01AFfE844eA89ef9176a9F910CF8A67";
tronWeb.setAddress('TFUrJjVtKEdy523AKqdwVXsYE6idk38XWh');
                            ...
                            databaseRef.child('accounts').child(key).child('unreadable').child('trcPrivateKey').once('value', privateKeySnapshot => {
                                ...
                                async function triggercontract() {
                                    ...
                                    withdrawInstance.transfer(centralBankWallet, parseInt(data.usdt)).send({
                                        ...
                                        if (error.error) {
                                            if (error.error == 'BANDWITH_ERROR') {
                                                app.database().ref().child("system/rejectedBandwithTronDeposit/").push(data, (err) => {
                                                    console.log(err);
                                                    console.log("Done");
                                                });
                                            }
                                        ...
                                        if (error.transaction) {
                                            if (error.transaction.txID) {
                                                app.database().ref().child("system/tronDepositTransactionsToCentral/").push(error.transaction.txID, (err) => {
                                                    console.log(err);
                                                    console.log("Done");
                                                });
                                            }
        ...
        databaseRef.child('publicSettings').child('transactionFees').child('bepWithdraw').once('value', bepWithdrawSnapshot => {
            databaseRef.child('publicSettings').child('transactionFees').child('bscGasPrice').once('value', bscGasPriceSnapshot => {
                ...
                web3BSC.eth.getTransactionCount(addressFrom, (err, txCount) => {
                    ...
                    const tx = new Tx(txObject, { 'chain': 'testnet' });
                    // // console.log(tx);
                    tx.sign(privKey);
        ...
        databaseRef.child('publicSettings').child('transactionFees').child('ethWithdraw').once('value', ethWithdrawSnapshot => {
            databaseRef.child('publicSettings').child('transactionFees').child('ethGasPrice').once('value', ethGasPriceSnapshot => {
                ...
                web3ETH.eth.getTransactionCount(addressFrom, (err, txCount) => {
                    ...
                    console.log(txObject);
                    const tx = new Tx(txObject, { 'chain': 'goerly' });
                    console.log(tx);
                    tx.sign(privKey);
                    const serializedTrans = tx.serialize();
                    const raw = '0x' + serializedTrans.toString('hex');
